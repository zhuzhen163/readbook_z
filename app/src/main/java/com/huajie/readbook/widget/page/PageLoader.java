...
import com.huajie.readbook.utils.IOUtils;
...
public abstract class PageLoader {
    ...
    private void initData() {
        ...
        mTextInterval = mSettingManager.getTextLineSpace();
...
//        mTextPaint.setTypeface(ZApplication.tf);
    ...
    public void setTextLine(int textSize) {
        // 设置文字相关参数
        setUpTextInterval(textSize);
    ...
    public void setUpTextInterval(int size){
        // 行间距(大小为字体的一半)
...
//            }
    ...
    //获取当前第一行文字
    public String getPagePosContent() {
        return mCurPage.lines.get(0);
    }

    //保存阅读记录
    public void saveRecord(boolean isCollected) {
        //书没打开，就没有记录
        if (!isBookOpen) return;
    ...
    public void saveBookMark() {
        ...
        markBean = new BookMarkBean();

        markBean.setBookId(mCollBook.get_id());
        markBean.setChapter(mCurChapterPos);
        markBean.setPagePos(mCurPage.position);
        markBean.setBooName(mCurPage.title);
        markBean.setTime(StringUtils.
                dateConvert(System.currentTimeMillis(), Constant.FORMAT_BOOK_DATE));
        if (this.markContent.contains(mCurPage.lines.get(0))) return;//相同位置不保存
        markContent = mCurPage.lines.get(0)+mCurPage.lines.get(1);
        markBean.setContent(markContent);
        //存储到数据库
        BookMarkHelpter.getsInstance().saveBookmark(markBean);
    }

    //打开书签
    public void openBookMark(BookMarkBean bean) {

        mBookRecord.setChapter(bean.getChapter());
        mBookRecord.setPagePos(bean.getPagePos());
        mBookRecord.setBookId(bean.getBookId());
        mCurChapterPos = bean.getChapter();
        mLastChapter = mCurChapterPos;
    }

    //打开具体书签
    public void openBookMarkPosition() {
        mCurPageList = loadPageList(mCurChapterPos);
        //进行预加载
        preLoadNextChapter();
        //加载完成
    ...
    public void openBook(CollBookBean collBook) {
        ...
        int latelyFollower = collBook.getLatelyFollower();
        if (latelyFollower != 0){
            mCurChapterPos = latelyFollower;
            mLastChapter = mCurChapterPos;
        }else {
    ...
    public void openChapter() {
        ...
        if (!isBookOpen) {
            isBookOpen = true;
            int latelyFollower = mCollBook.getLatelyFollower();
            if (latelyFollower != 0){
                mCurPage = getCurPage(0);
            }else {
                //可能会出现当前页的大小大于记录页的情况。
    ...
    /***********************************default method***********************************************/
    //通过流获取Page的方法
    List<TxtPage> loadPages(TxtChapter chapter, BufferedReader br,int chapterNum) {
        //生成的页面
...
//                  章节百分比
                    ...
                    percent = numberFormat.format(((float) (mCurPage.position + 1) / (float) mCurPageList.size()) * 100)+"%";
                }else {
//                  章节页码
